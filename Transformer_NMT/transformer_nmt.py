...
H_NUM = 8  # 多头注意力个数
D_MODEL = 64  # 输入、输出词向量维数
D_FF = 1024  # feed forward全连接层维数
DROPOUT = 0.1  # dropout比例
MAX_LENGTH = 60  # 语句最大长度

TRAIN_FILE_EN = 'TEDCorpus/train_en.txt'  # 训练集
TRAIN_FILE_CN = 'TEDCorpus/train_cn.txt'  # 训练集
DEV_FILE_EN = "TEDCorpus/dev_en.txt"  # 验证集
DEV_FILE_CN = "TEDCorpus/dev_cn.txt"  # 验证集
SAVE_FILE = 'save/model_final.pt'  # 模型保存路径
...
class PrepareData:
    def __init__(self, train_file_en, train_file_cn, dev_file_en, dev_file_cn):
        # 读取数据、分词
        self.train_en, self.train_cn = self.load_data(train_file_en, train_file_cn)
        self.dev_en, self.dev_cn = self.load_data(dev_file_en, dev_file_cn, 0)
        # 构建词表
        ...
        self.dev_en, self.dev_cn = self.word2id(self.dev_en, self.dev_cn, self.en_word_dict, self.cn_word_dict, False)
        # 划分批次、填充、掩码
    ...
    def load_data(self, en_path, cn_path, train_or_test=1):
        """
        ...
        with open(en_path, mode="r", encoding="utf-8") as f:
            for i, line in enumerate(f.readlines()):
                ...
                en.append(sent_en)
        with open(cn_path, mode="r", encoding="utf-8") as f:
            for i, line in enumerate(f.readlines()):
                if i > lines:
                    break
                sent_cn = line.strip()
...
pe = PositionEncoding(emb_dim, 0, max_seq_len)
...
class EncoderLayer(nn.Module):
    def __init__(self, size, self_attn, feed_forward, dropout):
        ...
        # Add_Norm作用连接multi和ffn
        self.sublayer = clones(Add_Norm(size, dropout), 2)
        # d_model
        self.size = size
...
class DecoderLayer(nn.Module):
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout):
        ...
        self.feed_forward = feed_forward
        self.sublayer = clones(Add_Norm(size, dropout), 3)
...
data = PrepareData(TRAIN_FILE_EN, TRAIN_FILE_CN, DEV_FILE_EN, DEV_FILE_CN)
